...
перші враження від бекплейна [olmaster c3804](https://www.aliexpress.com/i/32791168648.html): якість не вражає (лотки вимагають значної уваги при встромлянні, діоди статусу дуже різної яскравости тощо), але принаймні все наче працює, для домашнього використання придатне.

далі має бути азбац про те, чому mergerfs та snapraid, а не щось серйозніше, як zfs. якщо коротко: тому що «простота є необхідною передумовою надійности»<sup>†</sup>. так і не знайшов притомної детальної підказки з налаштування proxmox + mergerfs + snapraid в тенетах, доведеться експериментувати.


##### додавання дисків

як виявилося, один диск 2,5" на 500 гб я вже мав (у зовнішній кишені usb 3.0), ще два придбав задешево (30$ разом) на kijiji, для початку вистачить.
...
```
lsblk -o NAME,SIZE,MOUNTPOINT,TYPE,LABEL,MODEL
NAME     SIZE MOUNTPOINT  TYPE LABEL  MODEL
…
sdb    465.8G             disk        ST500LM021-1KJ152
└─sdb1 465.8G             part Data1  
sdc    465.8G             disk        ST500LM000-1EJ162
└─sdc1 465.8G             part Data2  
sdd    465.8G             disk        Hitachi_HTS545050A7E380
└─sdd1 465.8G             part Data3 
...
…
sdb    465.8G             disk        ST500LM021-1KJ152
└─sdb1 465.8G /mnt/disk/1 part Data1  
sdc    465.8G             disk        ST500LM000-1EJ162
└─sdc1 465.8G /mnt/disk/2 part Data2  
sdd    465.8G             disk        Hitachi_HTS545050A7E380
└─sdd1 465.8G /mnt/disk/3 part Data3 
```
...
створити теку, куди «об'єднаються» розділи на трьох дисках у бекплейні:
...
```
# sudo mergerfs -o allow_other,use_ino,nonempty /mnt/disk/\* /mnt/storage
```

для перевірки можна щось записати до `/mnt/storage`, і, залежно від стратегії розподілу mergerfs, фізично це опиниться на одному з трьох дисків, — і навпаки, `/mnt/storage` дає доступ до всього, що є на трьох дисках.

випробую швидкість:

```
$ dd if=/dev/zero of=/mnt/storage/dd_test_throughput.img bs=100M count=1 oflag=dsync status=progress
…
104857600 bytes (105 MB, 100 MiB) copied, 1392.09 s, 75.3 kB/s

```
що?! 75 кб/сек — це казна-що; чому? запис просто на диски (`/mnt/disk/*`) показує 70-110 мб/сек так само із `oflag=dsync`:

```
$ rm /mnt/storage/dd* && dd if=/dev/zero of=/mnt/disk/1/dd_test_throughput.img bs=100M count=1 oflag=dsync status=progress
…
104857600 bytes (105 MB, 100 MiB) copied, 0.932364 s, 112 MB/s

```

`oflag=dsync` змушує dd оминати кеш і чекати фізичного запису після кожного блоку. [сторінка mergerfs](https://github.com/trapexit/mergerfs#benchmarking) пропонує інший режим для випроби швидкодії (`oflag=nocache conv=fdatasync`), і тут все не так погано — 63 мб/сек:

```
$ rm /mnt/storage/dd* && dd if=/dev/zero of=/mnt/storage/dd_test_throughput.img bs=1M count=1024 oflag=nocache conv=fdatasync status=progress
915406848 bytes (915 MB, 873 MiB) copied, 4 s, 229 MB/s
…
1073741824 bytes (1.1 GB, 1.0 GiB) copied, 16.9268 s, 63.4 MB/s
```

перемонтував збірку mergerfs з опцією `direct_io` — отримав 2,3 мб/с:

```
$ sudo mergerfs -o defaults,allow_other,use_ino,nonempty,direct_io /mnt/disk/\* /mnt/storage
$ rm /mnt/storage/dd* ; dd if=/dev/zero of=/mnt/storage/dd_test_throughput.img bs=100M count=1 oflag=dsync status=progress
104857600 bytes (105 MB, 100 MiB) copied, 45 s, 2.3 MB/s
…
104857600 bytes (105 MB, 100 MiB) copied, 45.4328 s, 2.3 MB/s
```

не вражає… але реальні сценарії (копіювання файлів з ssd на /mnt/storage) показують значно кращу швидкість. можна додати збірку до fstab:

```
# MergerFS pool              
# <file system> <mount point>   <type>          <options>       <dump>  <pass>
/mnt/disk*      /mnt/storage    fuse.mergerfs   defaults,allow_other,use_ino,nonempty,direct_io,fsname=mergerfs,minfreespace=20G   0       0
```


##### smart

після підключення дисків варто перевірити звіти smart. по-перше — короткий тест для `sdb`, `sdc` та `sdd`:

```
# for X in b c d; do echo -n "/dev/sd${X}: "; sudo smartctl -t short /dev/sd${X} | grep "complete after"; done
/dev/sdb: Test will complete after Sun Jan 31 09:32:05 2021 EST
/dev/sdc: Test will complete after Sun Jan 31 09:33:05 2021 EST
/dev/sdd: Test will complete after Sun Jan 31 09:33:06 2021 EST
```

результати невтішні для hitachi (`sdd`) — добре, що я витяг диск із зовнішнього накопичувача з резервним архівом світлин, мав би неминучий сюрприз:

```
# for X in b c d; do echo -n "/dev/sd${X}: "; sudo smartctl --health /dev/sd${X} | grep "test result"; done
...
/dev/sdd: SMART overall-health self-assessment test result: FAILED!
```

для випроби диск згодиться: цікаво буде скористатися snapraid'ом для спроби відновлення, коли «гавкне». тим часом — нагода погратися ще трохи зі `smartctl` з набору `smartmontools` (хороші [підказки](https://www.smartmontools.org/wiki/TocDoc) є у вікі).

спробую перевірити весь диск:

```
# umount /dev/sdd1
# badblocks -v /dev/sdd > /tmp/badblocks-sdd.txt
```

добу потому: `badblocks` ще крутиться, знайшов 18 тис. «битих» блоків =( продовження процедури не має сенсу: нічого потрібного на диску немає, тож замість badblocks/fsck краще перестворити файлову систему заново з опцією `-c`, або навіть краще `-cc` для перевірки/маркування блоків:

```
# mkfs.ext4 -cc -L Data3 /dev/sdd1
...
Testing with pattern 0xff: 1.42% done, 20:49:50 elapsed. (62/0/0)
```

тут, `(62/0/0)` означає: 62 помилки читання, 0 помилок запису, 0 помилок… це вже третій прохід, і це зовсім не 18 тис. проблемних блоків. підозрюю, що проблема може бути не в дисках, а в дешевому китайському бекплейні, або в картці pcie -> sata… або в недостатньому живленні?

дивно, бо я перезавантажив сервер (все змонтувалося як слід), перезапустив тест smart (усі три диски ― passed), запустив у циклі запис/видалення файлів на всю ніч — і цього разу все працює без проблем. 
